/// The carved-wood auth look, kept here for the admin login alone.
///
/// This is the shell every auth screen used to share. The parent screens moved
/// to the play kit so a family sees one app from splash to celebration; the
/// admin portal is a separate session by design (UC-18) and is deliberately
/// left exactly as it was, so its login keeps the wooden look it shipped with.
///
/// Nothing in here is shared with the parent screens any more. Restyling the
/// admin portal is a separate job from restyling the app.
library;
...
class AdminWoodenField extends StatelessWidget {
  const AdminWoodenField({
...
class AdminAuthButton extends StatelessWidget {
...
class AdminAuthBanner extends StatelessWidget {
  const AdminAuthBanner({
